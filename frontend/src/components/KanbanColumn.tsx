...
export const KanbanColumn = ({
  ...
  return (
    <section
      ref={setNodeRef}
      className={clsx(
        "flex h-full min-h-[420px] flex-col rounded-2xl border border-[var(--stroke)] bg-[var(--surface-strong)] p-3 shadow-[var(--shadow)] transition",
        isOver && "ring-2 ring-[var(--accent-yellow)]"
      )}
      data-testid={`column-${column.id}`}
    >
      <div className="flex items-center gap-2 border-b border-[var(--stroke)] pb-3">
        <span className="h-5 w-1 shrink-0 rounded-full bg-[var(--accent-yellow)]" />
        <input
          value={column.title}
